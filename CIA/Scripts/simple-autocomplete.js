...
angular.module('simple-autocomplete', [])
	.directive('autocomplete', ['autocomplete-keys', '$window', '$timeout', function (Keys, $window, $timeout) {
	    var template =
		'<input id="{{id}}" type="text" class="autocomplete-input" placeholder="{{placeHolder}}"' +
			' ng-class="inputClass"' +
			' ng-model="searchTerm"' +
	    ...
	    return {
	        ...
	        controller: function ($scope) {
	            ...
	            $scope.processSearchTerm = function (term) {
	                if (term.length > 0) {
	                    ...
	                    if (matchingOptions.length == 0) {
	                        console.log('no matching options found', $scope.matchingOptions);
	            ...
	            $scope.findMatchingOptions = function (term) {
	                if ($scope.id.toLowerCase() == "meetingname") {
	                    if (!$scope.options) {
	                        throw 'You must define a list of options for the autocomplete or it took too long to load';
	                    }
	                    return $scope.options.filter(function (option) {
	                        ...
	                        if (searchProperty) {
	                            ...
	                            //return lowerCaseOption.startsWith(lowerCaseTerm);
	                        }
	                        return false;
	                    });
	                }
	                    ...
	                    return $scope.options.filter(function (option) {
	                        ...
	                        if (searchProperty) {
	                            ...
	                            if (!String.prototype.startsWith) {
	                                //supports for IE 11
	                                return lowerCaseOption.lastIndexOf(lowerCaseTerm, 0) === 0;
	                            }
	                            else {
	                                // not supports for IE 11
	                                return lowerCaseOption.startsWith(lowerCaseTerm);
	                            }
	                        }
	                        return false;
